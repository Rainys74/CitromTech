...
#ifdef CT_DEBUG
#define CT_ASSERT_THROW_APP_ERROR(x) DEBUG_BREAK()
#else
#define CT_ASSERT_THROW_APP_ERROR(x) Citrom::Assertion::BreakApplication(x)
#endif

#if defined(CT_DEBUG) || defined(CT_RELEASE)
#define CT_CORE_ASSERT(x, y) if (!(x)) {CT_CORE_FATAL("ASSERT: ""{}({}): error: {} line: {} in function: {}()", __FILE__, __LINE__, #y, #x, __func__); CT_ASSERT_THROW_APP_ERROR(y);}
#define CT_ASSERT(x, y) if (!(x)) {CT_FATAL("ASSERT: ""{}({}): error: {} line: {} in function: {}()", __FILE__, __LINE__, #y, #x, __func__); CT_ASSERT_THROW_APP_ERROR(y);}
